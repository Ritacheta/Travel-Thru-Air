...
import { Autocomplete, TextField, CardContent, CardActions, Card, Select, FormControl, MenuItem, Box, InputLabel, Button } from "@mui/material";
	...
	const handleFlightSearch = (e) => {
		...
		fetch(
			...
			.then((response) => {
				...
				searched_flights = response.flights;
		...
		<div>
			<Card>
				<CardContent>
					...
					>
						<Select labelId="demo-simple-select-label" id="demo-simple-select" name="booking_type" onChange={handleChange} value={search_option.booking_type}>
					...
					>
						<TextField label="Total Person" variant="outlined" onChange={handleChange} />
					</FormControl>
					<FormControl
						sx={{
							width: "20%",
					...
					>
						<Select labelId="demo-simple-select-label" id="demo-simple-select" name="booking_class" onChange={handleChange} value={search_option.booked_class}>
					...
					>
						<Autocomplete
							...
							autoHighlight
							renderInput={(params) => (
								<TextField {...params} label="Where from?" onChange={handleChange} value={search_option.flight_from} name="flight_from" />
							)}
						/>
						<CompareArrowsIcon
							sx={{
						...
						<Autocomplete
							...
							options={airports}
							autoHighlight
							renderInput={(params) => <TextField {...params} label="Where to?" onChange={handleChange} value={search_option.flight_to} name="flight_to" />}
						/>
			...
			<Box>
				<FlightCard flights={searched_flights} searched_option={search_option} />
			</Box>
		</div>
	);
}
